...
public class OnlineInputField : MonoBehaviour
{
    public string text;
    ...
    void Update()
    {
        GameObject inputField = otherInputField.GetComponentInParent<Transform>().gameObject;
        if (inputField.activeInHierarchy)
        {
            if(otherInputField.text != text)
            {
                outputPlaceHolder.SetActive(false);
                text = otherInputField.text;
                outputText.text = convertText(otherInputField.text);
            }

            if(otherInputField.text == "")
            {
                outputPlaceHolder.SetActive(true);
            }
        }
        ...
        else//chinese
        {
            ...
            for (int i = 0; i < sentences.Length; i++)
            {
                ...
                else
                {
                    ...
                    {
                        if (stringWords[k].Length > 0 && stringWords[k].Length <= 5)
                        {
